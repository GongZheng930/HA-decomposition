...
def hh(borr_cons, Va_p, a_grid, we, T, r, beta, eis, frisch, vphi, re_shock):
    '''Single backward step via EGM.'''
    uc_nextgrid = beta * (1 + r) * Va_p
    c_nextgrid, n_nextgrid = cn(uc_nextgrid, we[:, np.newaxis], eis, frisch, vphi)

    lhs = c_nextgrid + a_grid[np.newaxis, :] / (1 + r) - we[:, np.newaxis] * n_nextgrid  - T[:, np.newaxis]
    rhs = a_grid[np.newaxis, :] + re_shock
    ...
    if iconst[0].size != 0 and iconst[1].size != 0:
        c[iconst], n[iconst] = solve_cn(we[iconst[0]],
                                        rhs[iconst] + T[iconst[0]] - borr_cons / (1 + r) ,
